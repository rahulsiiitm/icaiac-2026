    ...
    <main className="min-h-screen bg-cream-100 p-8 text-charcoal font-sans">
      <div className="max-w-7xl mx-auto">
        <header className="flex justify-between items-end mb-12">
            ...
            <button className="bg-charcoal text-white px-8 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-gold hover:text-charcoal transition-all shadow-xl rounded-sm">
                ...
                {filteredList.map((reg) => (
                    <tr key={reg.id} className="hover:bg-cream-100/30 transition-colors">
                        ...
                        <td className="p-5">
                            ...
                            {reg.payment?.receiptUrl && <a href={reg.payment.receiptUrl} target="_blank" className="text-[9px] underline opacity-40 hover:opacity-100">View Receipt</a>}
                        </td>
                        <td className="p-5 text-right">
                            <div className="flex items-center justify-end gap-4">
                                <span className={`px-2 py-1 text-[8px] font-bold rounded-full uppercase ${reg.payment?.status === 'VERIFIED' ? 'bg-green-100 text-green-700' : 'bg-gold/10 text-gold'}`}>{reg.payment?.status || "NOT PAID"}</span>
                                {reg.payment?.status === "PENDING" && (
                                    <button onClick={() => verifyPayment(reg.payment.id)} className="bg-gold text-charcoal px-4 py-2 text-[9px] font-bold uppercase hover:bg-charcoal hover:text-white transition-all rounded-sm shadow-md">Verify</button>
                                )}
                            </div>
                        </td>
                    </tr>
