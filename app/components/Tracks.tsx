...
const TRACKS = [
  ...
  { id: "5", title: "AI & Machine Learning", borderColor: "#D4AF37" }, // CENTER
...
export default function Tracks() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Hexagon Pop Animation (Kept this as it is the main visual feature)
      gsap.fromTo(
        ".hexagon-wrapper",
        { scale: 0.5, opacity: 0 },
        {
  ...
  return (
    <div className="bento-card col-span-1 md:col-span-4 bg-linear-to-br from-charcoal-light to-black border border-white/10 p-8 flex flex-col items-center justify-center text-center relative overflow-hidden group">
      ...
      <p className="relative z-10 font-sans text-white/50 text-xs leading-relaxed mb-0">
        Accepted papers will be submitted for inclusion into IEEE Xplore subject to meeting scope and quality requirements.
      </p>
    </div>
  );
}
...
function DownloadButton({ href, label }: { href: string; label: string }) {
  return (
    <a 
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="flex items-center justify-center gap-2 py-3 border border-white/30 text-[10px] font-bold uppercase tracking-widest text-white hover:bg-white hover:text-charcoal transition-all"
    >
    ...
    >
      <div className="relative w-full h-full transition-all duration-300 ease-out group-hover:scale-[1.05] group-hover:drop-shadow-[0_0_30px_rgba(212,175,55,0.3)]">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-8 text-center z-20">
          <span className="font-serif text-6xl font-bold mb-1 transition-colors duration-300 absolute top-6 opacity-10" style={{ color: track.borderColor }}>
            {track.id}
          </span>
          <p className="font-sans text-sm font-bold leading-tight text-white uppercase tracking-widest relative z-10 mt-6 drop-shadow-lg">
            {track.title}
          </p>
