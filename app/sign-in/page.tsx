...
export default function SignInPage() {
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    if (res?.ok) router.push("/dashboard");
      ...
      alert("Invalid email or password. If you signed up with Google, use the Google button below.");
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="bg-charcoal min-h-screen flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white/5 border border-white/10 p-10 rounded-sm backdrop-blur-md shadow-2xl">
        ...
        <form onSubmit={handleCredentialsLogin} className="space-y-6 mb-4">
        ...
        <div className="text-right mb-8">
          <Link href="/forgot-password" className="text-[10px] text-white/30 hover:text-gold transition-colors uppercase tracking-widest">
            Forgot Password?
          </Link>
        </div>
        ...
        <p className="text-center text-[10px] uppercase tracking-widest text-white/30">
          Don&apos;t have an account? <Link href="/sign-up" className="text-gold hover:underline">Sign Up</Link>
        </p>
      </div>
    </div>
  );
